...
import { generateShopBarcode, normalizeBarcode } from "@/lib/shop/inventory/barcode";
import {
  billNumberToBarcodeValue,
  normalizeBillScan,
} from "@/lib/shop/invoices/bill-barcode";
import { nextShopBillNumber } from "@/lib/shop/invoices/bill-number";
import { ensureShopBranchSchema, backfillOrgBranchIds } from "@/lib/shop/schema/ensure-shop-branch-schema";
import { branchWhere, type BranchScope } from "@/lib/shop/branch/branch-context";
import {
  customerSearchWhere,
} from "@/lib/shop/customers/customer";
import {
  getCustomerRaw,
  upsertCustomerRaw,
...
import {
  ...
  parseSaleItemsJson,
  type SaleLineInput,
} from "@/lib/shop/inventory/inventory-analytics";
...
} from "@/lib/shop/inventory/sale-inventory-context";
...
import { hasKot, hasRecipeConsumption } from "@/lib/shop/branch/sector-mode";
import {
...
export async function createInventoryItem(input: {
  ...
  await ensureCatalogSchema();
  await ensureShopBranchSchema(input.organizationId);
  const { ensureDefaultBranch } = await import("@/lib/shop/branch/branch-context");
...
export type BulkImportItemInput = import("@/lib/shop/inventory/inventory-bulk-csv").BulkImportRow;
...
export async function bulkImportInventoryItems(input: {
  ...
  const { groupImportRowsIntoProducts } = await import("@/lib/shop/inventory/inventory-bulk-csv");
