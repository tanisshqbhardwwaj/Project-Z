...
export async function createShopSale(input: {
  ...
  try {
    const { enqueueSyncOutbox } = await import("@/lib/desktop/local-mode");
...
export async function createInventoryItem(input: {
  ...
  if (org?.businessType === "SHOPKEEPER") {
    const { inventorySkuCapForPlan } = await import("@/lib/billing/entitlements");
    const cap = inventorySkuCapForPlan(org.plan);
    if (cap != null) {
      ...
      if (count >= cap) {
        throw new Error(
          `Basic plan allows up to ${cap} products. Upgrade in Settings → Billing for unlimited inventory.`
        );
      }
    }
  }
